        ...
        for key in synapses:
            if synapses[key].Get_Target_Neuron_Name() == self.Get_Name():
                for neuron in neurons:
                    if neurons[neuron].Get_Name() == synapses[key].Get_Source_Neuron_Name():
                        self.Allow_Presynaptic_Neuron_To_Influence_Me(synapses[key].Get_Weight(), neurons[neuron].Get_Value())
            self.Threshold()
